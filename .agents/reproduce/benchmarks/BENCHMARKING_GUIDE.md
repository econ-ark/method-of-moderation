...
./.agents/reproduce/benchmarks/benchmark.sh --min

# Benchmark full reproduction (all tests, paper, notebooks)
./.agents/reproduce/benchmarks/benchmark.sh

# With notes
./.agents/reproduce/benchmarks/benchmark.sh --min --notes "Testing M1 Max performance"
```
...
cat .agents/reproduce/benchmarks/results/latest.json | jq '.system'

# Check duration
cat .agents/reproduce/benchmarks/results/latest.json | jq '.duration_seconds'
```
...
```bash
# Before optimization
./.agents/reproduce/benchmarks/benchmark.sh --min --notes "Before optimization"

# After optimization
./.agents/reproduce/benchmarks/benchmark.sh --min --notes "After optimization"
...
```bash
# On laptop
./.agents/reproduce/benchmarks/benchmark.sh --min --notes "MacBook Pro M1 2021"

# On desktop
./.agents/reproduce/benchmarks/benchmark.sh --min --notes "AMD Ryzen 9 5900X"
```
...
./.agents/reproduce/benchmarks/benchmark.sh --min --notes "UV environment"

# With Conda
conda activate moderation
./.agents/reproduce/benchmarks/benchmark.sh --min --notes "Conda environment"
```
...
```yaml
- name: Run Benchmark
  run: ./.agents/reproduce/benchmarks/benchmark.sh --min

- name: Upload Results
  uses: actions/upload-artifact@v3
  with:
    name: benchmark-results
    path: .agents/reproduce/benchmarks/results/latest.json
```
