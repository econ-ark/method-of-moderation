...
ACTIVATION_CODE='
# Auto-activate Method of Moderation virtual environment (platform and architecture-specific)
if [ -z "${VIRTUAL_ENV:-}" ]; then
    # Determine workspace directory
    MOM_WORKSPACE="/workspace"
    if [ ! -d "$MOM_WORKSPACE" ]; then
        # Fallback: try common locations
        for dir in "/workspace" "$HOME/method-of-moderation" "$HOME/workspace"; do
    ...
    case "$(uname -s)" in
        Darwin)
            ...
            if [ -f "$MOM_WORKSPACE/.venv-darwin-$MOM_ARCH/bin/activate" ]; then
                MOM_VENV="$MOM_WORKSPACE/.venv-darwin-$MOM_ARCH"
            fi
            ;;
        Linux)
            if [ -f "$MOM_WORKSPACE/.venv-linux-$MOM_ARCH/bin/activate" ]; then
                MOM_VENV="$MOM_WORKSPACE/.venv-linux-$MOM_ARCH"
            fi
            ;;
    esac

    # Activate if found
    if [ -n "$MOM_VENV" ] && [ -f "$MOM_VENV/bin/activate" ]; then
        source "$MOM_VENV/bin/activate"
        export PYTHONPATH="$MOM_WORKSPACE/code:$PYTHONPATH"
    fi
fi'
...
if [ -f "$HOME/.bashrc" ] || [ "$(uname -s)" = "Linux" ]; then
    ...
    if ! grep -q "Auto-activate Method of Moderation" "$HOME/.bashrc" 2>/dev/null; then
...
if [ -f "$HOME/.zshrc" ] || [ "$(uname -s)" = "Darwin" ]; then
    ...
    if ! grep -q "Auto-activate Method of Moderation" "$HOME/.zshrc" 2>/dev/null; then
...
echo "   export PYTHONPATH=\"$WORKSPACE_DIR/code:\$PYTHONPATH\""
echo ""
