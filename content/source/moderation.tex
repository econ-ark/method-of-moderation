...
%
% NOTE (2026): This is the LEGACY LaTeX source. The active version of the
% paper lives in MyST under content/paper/moderation.md. The chi/logit
% convention used here is the NEGATIVE of the MyST source:
%   - This file:    chi = log((1 - kappa) / kappa)  (i.e. negative logit)
%   - MyST source:  chi = log(omega / (1 - omega))  (standard positive logit)
% Both are internally consistent with their own reconstruction formulas, but
% they imply opposite-signed expressions for c_real. The code in
% code/moderation.py implements the MyST positive-logit convention. Do not
% reconcile by transplanting equations between the two without checking the
% sign convention.
...
\begin{authorsinfo}
  ...
  \name{Wu: Weifeng Wu, Fannie Mae, Washington DC, \href{mailto:weifeng\_wu@fanniemae.com}.}
\end{authorsinfo}
...
In the stochastic case, where analytical solutions are generally not available, a common approach is to use low-order polynomial splines that exactly match the function (and maybe some derivatives) at a finite set of gridpoints, and then to assume that interpolated or extrapolated versions of that spline represent the function well at the continuous infinity of unmatched points.
...
% === END CONTENT FROM C:\Users\alujan\GitHub\llorracc\SolvingMicroDSOPs-Latest\Equations\distRisky.tex ===
 \noindent because in this case \cite{Merton1969} and \cite{Samuelson1969} showed that for a consumer without labor income (or with perfectly forecastable labor income) the consumption function is linear, with an MPC.\footnote{See \handoutC{CRRA-RateRisk} for a derivation.}
