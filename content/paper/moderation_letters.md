...
for a constant minimal marginal propensity to consume $\MPCmin$.[^mpc-min-definition] We similarly define $\hNrmPes$[^pessimist-human-wealth] as 'minimal human wealth,' the present discounted value of labor income if the shocks were to take on their worst value in every future period $\tranShk_{t+n} = \tranShkMin$ $\forall~n>0$ (which we define as corresponding to the beliefs of a 'pessimist'). We will call a 'realist' the consumer who correctly perceives the true probabilities of the future risks and optimizes accordingly.
...
Differentiating {eq}`eq:cFuncHi` with respect to $\mNrm$ yields a moderation form for the marginal propensity to consume:
